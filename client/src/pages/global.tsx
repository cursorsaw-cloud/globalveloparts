    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      <section className="relative overflow-hidden bg-[linear-gradient(180deg,#f8fafc_0%,#eef2f7_100%)]">
        ...
        <div className="section-shell relative z-10 py-20 md:py-24 lg:py-28">
          <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(22rem,29rem)] lg:gap-12">
            <div className="max-w-[45rem]">
              <span className="section-kicker text-secondary" data-testid="text-global-badge">
                {t("global.badge")}
              </span>
              <h1 className="mt-5 text-[3rem] font-extrabold leading-[0.96] tracking-[-0.05em] text-slate-950 sm:text-[4rem] lg:text-[5rem]" data-testid="text-global-title">
                {t("global.title")}
              </h1>
              <p className="mt-7 max-w-[39rem] text-[1.05rem] leading-8 text-slate-600 md:text-[1.16rem]" data-testid="text-global-description">
                {t("global.desc")}
              </p>
              <div className="mt-10 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                {stats.map((stat) => (
                  <div key={stat.label} className="surface-panel rounded-[1.5rem] px-5 py-5" data-testid={`card-global-stat-${stat.label}`}>
        ...
        <div className="section-shell">
          ...
          <div className="mt-14 rounded-[2rem] bg-[linear-gradient(135deg,#0f172a_0%,#1e293b_100%)] px-7 py-8 text-white shadow-[0_30px_90px_-40px_rgba(15,23,42,0.75)] md:px-10 md:py-10">
            <div className="grid items-center gap-6 lg:grid-cols-[minmax(0,1fr)_auto_auto]">
              ...
              <Button asChild size="lg" className="h-14 rounded-full px-8 text-base shadow-[0_20px_55px_-25px_rgba(37,99,235,0.65)]">
                <a href="/products" data-testid="button-global-products">
              ...
              <Button asChild size="lg" variant="outline" className="h-14 rounded-full border-white/15 bg-white/5 px-8 text-base text-white hover:bg-white/10 hover:text-white">
                <a href="/#contact" data-testid="button-global-contact">
                  {t("global.cta.secondary")}
                </a>
              </Button>
            </div>
          </div>
